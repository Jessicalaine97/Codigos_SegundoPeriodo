...
/*Recupere os dados dos carros que não participaram de nenhum acidente.*/

select placa from participou;

select *
from carro
where placa not in (select placa from participou);


/*Recupere os dados do(s) carro(s) que teve(tiveram) o maior valor de dano, juntamente com o valor do dano.*/
select distinct c.*, valor_dano
from carro c inner join participou p on c.placa=p.placa
where valor_dano=(select max(valor_dano) from participou);

/*Recupere os dados do(s) carro(s) que teve(tiveram) o menor valor de dano, juntamente com o valor do dano.*/
select distinct c.*, valor_dano
from carro c inner join participou p on c.placa=p.placa
where valor_dano=(select min(valor_dano) from participou);

/*Recupere as informações do(s) carro(s) que participou(aram) do maior número de ocorrências.*/
select distinct c.*, count(numero_ocorrencia)
from carro c inner join participou p on c.placa=p.placa
where count(numero_ocorrencia)=(select max(count(numero_ocorrencia)) from participou);

/*Recupere a soma, a média, o valor mínimo e o valor máximo do valor do dano ocasionado por cada veículo.*/
select c.placa, modelo, min(valor_dano) as valor_min, max(valor_dano) as valor_max, avg(valor_dano) as media, sum(valor_dano) as soma 
from carro c inner join participou p on c.placa=p.placa
group by c.placa;

/*Recupere todos as informações do acidente mais antigo.*/
select *
from acidente
where data = (select min(data) from acidente);

/*Recuperar os dados do carro mais antigo que se envolveu em algum acidente.*/
select *
from carro 
where ano=(select min(ano) from carro c inner join participou p on c.placa=p.placa);


---
View:

create view carro_participou as select c.*
from carro c inner join participou on c.placa=participou.placa;

select * from carro_participou;
