...
/*Traga todas as informações dos produtos que não foram vendidos. Utilize junção externa.*/
select p.* from produto p left join produto_vendido v on p.id_produto=v.id_produto where v.id_produto is null;

/*Traga todas as informações dos clientes que não efetuaram nenhuma compra. Utilize junção externa.*/
select c.* from cliente c left join venda v on c.id_cliente=v.id_cliente where v.id_cliente is null;

/*Traga as informações do produto que possui maior quantidade em estoque*/
select* from produto order by estoque desc;
select * from produto where estoque = (select max(estoque) from produto);

/*Traga todas as informações, juntamente com preço do produto que possui o menor valor.*/
select * from produto where preco_unitario = (select min(preco_unitario) from produto);

/*Recupere o número da venda e a quantidade de produtos que foram vendidos por venda. 
Traga apenas as vendas que tiveram mais de 5 produtos vendidos. 
Ordene da maior para menor quantidade de produtos vendidos.*/
select v.nr_venda, sum(quantidade) as soma_venda
from produto_vendido pv inner join venda v on pv.nr_venda=v.nr_venda
group by v.nr_venda having soma_venda>5 order by soma_venda desc;
...
/* Recupere as informações dos clientes e a quantidade de compras que eles efetuaram. 
Caso o cliente não tenha efetuado nenhuma compra, o valor 0 deverá ser mostrado na frente das 
informações do cliente.*/
select c.*, count(v.id_cliente)
from cliente c left join venda v on c.id_cliente = v.id_cliente
group by c.id_cliente;

/*GROUP BY CHAVE PRIMARIA*/